...
class PlayerTokenTest {
    ...
    void advanceTest() {
        ...
        assertEquals(0, token.getFaithPoints());

        token.advance();

        assertEquals(1, token.getFaithPoints());

        for (int i=0; i<4; i++) {
            token.advance();
        }

        assertEquals(5, token.getFaithPoints());
    }

    @Test
    void concedeTest() {
        VaticanRoute route = new VaticanRoute(3);
        PlayerToken token = (PlayerToken) route.getTokenList().get(0);
